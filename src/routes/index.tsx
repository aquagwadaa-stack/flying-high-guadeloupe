...
function Hero() {
  return (
    <section className="relative min-h-[100svh] flex items-end pb-20 lg:pb-32 overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-5 lg:px-8 w-full pt-24">
        <div className="max-w-3xl text-white animate-rise">
          ...
          <div className="flex flex-wrap gap-3">
            <Link to="/reserver" className="inline-flex items-center gap-2 h-13 px-7 py-4 rounded-full bg-lagoon text-white font-semibold text-base hover:brightness-110 active:scale-95 transition-all shadow-2xl shadow-lagoon/30">
              Réserver une séance <ArrowRight className="size-4" />
            </Link>
            <Link to="/decouvrir" className="inline-flex items-center h-13 px-7 py-4 rounded-full bg-white/10 backdrop-blur-md text-white font-semibold text-base border border-white/30 hover:bg-white/20 transition-all">
      ...
      {/* Trajectory SVG */}
      <svg className="absolute bottom-0 left-0 w-full h-32 text-paper" viewBox="0 0 1440 120" preserveAspectRatio="none" aria-hidden>
...
function Reassurance() {
  const items = [
    ...
    { icon: MapPin, title: "Au Helleux, Sainte-Anne", desc: "Cadre tropical exceptionnel." },
  ];
  return (
        ...
        <div className="space-y-8">
          <div>
            <span className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[0.2em] text-lagoon mb-4">
              <span className="h-px w-6 bg-lagoon" /> L’expérience
            </span>
          ...
          <p className="text-lg text-muted-foreground leading-relaxed max-w-prose">
            Envie de voler, de vous dépasser ou simplement de partager un moment inoubliable ? L’équipe Trapez’cool vous accompagne pas à pas, de vos premiers élans jusqu’à vos premières figures. Aucune expérience n’est nécessaire : venez comme vous êtes, nous nous occupons du reste.
          </p>
          <ul className="space-y-5">
            {[
              { c: "lagoon", t: "Une progression adaptée à chacun", d: "Chaque élève avance à son rythme avec des conseils personnalisés." },
              { c: "coral", t: "Accessible aux enfants et aux adultes", d: "Dès 7 ans, sans limite d’âge maximale." },
              { c: "solar", t: "Une ambiance sportive et détendue", d: "Une équipe encourageante, jamais dans la performance imposée." },
            ].map((it) => (
              <li key={it.t} className="flex gap-4">
                <span className={`size-6 mt-1 rounded-full flex items-center justify-center shrink-0 bg-${it.c}/15`}>
    ...
    <section className="py-24 lg:py-32 px-5 lg:px-8 bg-secondary/40">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-14">
          <div>
            <span className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[0.2em] text-lagoon mb-4">
              <span className="h-px w-6 bg-lagoon" /> Choisissez votre expérience
            </span>
            <h2 className="font-display text-4xl lg:text-5xl font-bold tracking-tight text-balance max-w-2xl">
              Une formule pour chaque envie.
            </h2>
          </div>
          <Link to="/seances" className="inline-flex items-center gap-2 text-lagoon font-semibold hover:gap-3 transition-all">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {cards.map((c, i) => (
            ...
            >
              <div className={`size-12 rounded-2xl flex items-center justify-center mb-6 bg-${c.color}/10`}>
              ...
              <div className="flex items-end justify-between border-t border-border pt-5">
                <div>
                  <div className="text-xs text-muted-foreground uppercase tracking-wide font-medium">{c.duration}</div>
...
function Steps() {
  const steps = [
    { n: "01", t: "Accueil et présentation", d: "Vous êtes accueilli par l’équipe qui vous explique l’activité et le matériel." },
    { n: "02", t: "Échauffement et consignes", d: "Mise en condition douce et explication des gestes essentiels au sol." },
    { n: "03", t: "Premiers passages au trapèze", d: "Vous montez à l’échelle pour vos premières sensations, en toute sécurité." },
    { n: "04", t: "Progression selon vos sensations", d: "Nous adaptons les exercices et tentons vos premières figures." },
  ];
  return (
      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16 max-w-2xl mx-auto">
          ...
          <h2 className="font-display text-4xl lg:text-5xl font-bold tracking-tight text-balance">
            Du sol jusqu’à votre premier vol.
          </h2>
        </div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-6">
          {steps.map((s) => (
            <div key={s.n} className="relative">
              <div className="font-display text-[120px] leading-none font-bold text-white/5 absolute -top-8 -left-2 select-none">{s.n}</div>
              <div className="relative">
  ...
  return (
    <section className="px-5 lg:px-8 pt-24 lg:pt-32 pb-0">
      <div className="max-w-7xl mx-auto relative rounded-[2.5rem] overflow-hidden bg-gradient-to-br from-lagoon via-turquoise to-lagoon p-10 lg:p-20 text-white">
        <svg className="absolute inset-0 w-full h-full opacity-20" viewBox="0 0 800 400" preserveAspectRatio="none" aria-hidden>
        ...
        <div className="relative grid lg:grid-cols-[1.5fr_1fr] gap-10 items-center">
          <div>
            ...
            <p className="text-white/90 text-lg max-w-xl mb-8">
              Choisissez votre créneau, payez en ligne en toute sécurité et recevez vos informations pratiques par e-mail.
            </p>
            <div className="flex flex-wrap gap-3">
              <Link to="/reserver" className="inline-flex items-center gap-2 h-13 px-7 py-4 rounded-full bg-white text-midnight font-semibold hover:bg-solar transition-colors">
                Voir les prochaines disponibilités <ArrowRight className="size-4" />
              </Link>
              <a href="https://wa.me/590690193428" className="inline-flex items-center h-13 px-7 py-4 rounded-full bg-white/15 backdrop-blur text-white font-semibold border border-white/30 hover:bg-white/25 transition-all">
                WhatsApp
              </a>
            </div>
          </div>
          {next && (
            <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20">
              <div className="text-xs uppercase tracking-widest font-bold text-white/70 mb-3">Prochaine séance</div>
              <div className="font-display text-2xl font-bold capitalize mb-1">{formatDateLong(next.date)}</div>
              <div className="flex items-center gap-2 text-white/80 mb-4">
                <Clock className="size-4" /> {next.time.replace(":", " h ")} – {next.endTime.replace(":", " h ")}
              </div>
